...
class CatController(
    ...
    @AdminAccess
    @PostMapping("/random")
    @Operation(summary = "Store a new random cat.", security = [SecurityRequirement(name = "Bearer")])
    @ApiResponse(responseCode = "201", description = "Cat created")
    @ResponseStatus(HttpStatus.CREATED)
    ...
    @GetMapping("/{id}")
    @Operation(summary = "Retrieve a cat.", security = [SecurityRequirement(name = "Bearer")])
    @ApiResponses(
        value = [
    ...
    @GetMapping("")
    @Operation(
        summary = "Retrieve all cats.", security = [SecurityRequirement(name = "Bearer")]
    )
